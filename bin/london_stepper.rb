...
module LondonStepper
  ...
  class Server < Thor
    ...
    source_root File.realpath((__FILE__) + '/../../templates/server')

    desc "setup", "Setup all progs and utilities to fresh server"
    long_desc <<-LONGDESC
      Long description
    LONGDESC
  ...
  # Manage rails apps on remote server
  class App < Thor
    include Thor::Actions
    source_root File.realpath((__FILE__) + '/../../templates/app')

    desc "create <name>", "Create Rails app project"
    def create(name)

    end

    desc "destroy <name>", "Remove Rails app project"
    def destroy(name)

    end

    desc "index", "List of all apps"
    def index
  ...
  class CLI < Thor
    desc "server <command> <args>", "Manage remote server"
    subcommand 'server', Server

    desc "app <command> <args>", "Manage rails apps on remote server"
    subcommand 'app', App
  end
end

LondonStepper::CLI.start(ARGV)
=begin
# Command-line tool to manage servers
class LondonStepper < Thor
  include Thor::Actions
